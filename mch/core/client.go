...
import (
	...
	"github.com/chunbaise/wechat/internal/debug/mch/api"
	wechatutil "github.com/chunbaise/wechat/util"
)
...
func (clt *Client) PostXML(url string, req map[string]string) (resp map[string]string, err error) {
	...
	case "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers", "https://api2.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers", // 企业付款
		...
		// TODO(huangzhenqiang): 这几个接口没有标准的 appid 和 mch_id 字段，需要用户在 req 里填写全部参数
		// TODO(huangzhenqiang): 通读整个支付文档, 可以的话重新考虑逻辑
	default:
		if req["appid"] == "" {
			req["appid"] = clt.appId
		}
	...
	if err = utxml.EncodeXMLFromMap(buffer, req, "xml"); err != nil {
		return nil, err
	}
	body := buffer.Bytes()
...
func (clt *Client) postXML(url string, body []byte, reqSignType string) (resp map[string]string, needRetry bool, err error) {
	...
	if signatureHave == "" {
		// TODO(huangzhenqiang): 在适当的时候更新下面的 case
		switch url {
		default:
